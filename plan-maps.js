...
(function (root) {
  ...
  /* PRIMARY_MAP[track][stage] -> href of the tool that must lead the plan.
     Revives the 3rd question, which NX.tools() previously ignored. */
  var PRIMARY_MAP = {
    men: {
      starting:       '/testosterone-test',
      normal_but_off: '/testosterone-levels-by-age',
      have_bloods:    '/testosterone-levels-by-age'
    },
    women: {
      starting:       '/menopause-stage-quiz',
      normal_but_off: '/menopause-stage-quiz',
      have_bloods:    '/perimenopause-blood-test'
    }
  };
  ...
  var NX = (function () {
    ...
    function articles(track, goal) {
      ...
      if (track === 'women') {
        ...
        var wlist = pw.slice(0, 4).map(function (o) { return { href: o.a.href || ('/womens-articles/' + o.a.slug), label: o.a.name }; });
        /* the women's library has no piece on testosterone in women; pin it where it genuinely belongs */
        DATA_WOMEN_ART.forEach(function (a) {
          if (!a.pin || a.pin.indexOf(goal) === -1) return;
          var href = a.href || ('/womens-articles/' + a.slug);
          for (var i = 0; i < wlist.length; i++) { if (wlist[i].href === href) return; }
          wlist.push({ href: href, label: a.name });
        });
        return { list: wlist, seeAll: '/womens-hormone-health' };
      }
    ...
    function tools(track, goal, stage) {
      ...
      var scored = pool.map(function (t) {
        return { t: t, c: (t.goals.indexOf(goal) !== -1 ? 2 : 0) + (t.track === track ? 1 : 0) + (t.boost || 0) };
      }).sort(function (a, b) { return b.c - a.c; });
      var list = scored.slice(0, 4).map(function (o) { return { name: o.t.name, blurb: o.t.blurb, href: '/' + o.t.slug }; });
      /* the 3rd question finally matters: a stage-specific tool leads the plan */
      var pri = (PRIMARY_MAP[track] && PRIMARY_MAP[track][stage]) || null;
      if (pri) {
        var hit = null;
        for (var i = 0; i < pool.length; i++) { if ('/' + pool[i].slug === pri) { hit = pool[i]; break; } }
        if (hit) {
          list = list.filter(function (o) { return o.href !== pri; });
          list.unshift({ name: hit.name, blurb: hit.blurb, href: '/' + hit.slug });
          list = list.slice(0, 4);
        }
      }
